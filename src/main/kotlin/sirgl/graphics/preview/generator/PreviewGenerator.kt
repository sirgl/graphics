...
private fun showGaussMatrix(): BufferedImage {
    val matrix = generateGaussMatrix(27, 2.0f)
    val max = matrix.values.max()!!
    val size = matrix.width
    val img = BufferedImage(size, size, BufferedImage.TYPE_INT_RGB)
    for (y in (0 until size)) {
        for (x in (0 until size)) {
            val value = matrix.getXY(x, y)
